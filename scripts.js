  ...
  async function cargarDatosStrava(accessToken) {
    try {
      ...
      if (actividades.length > 0) {
        ...
        const tiempoStr = hh > 0 ? `${hh}:${mm}:${ss}` : `${mm}:${ss}`;
        const elTiempo = document.getElementById('actTiempo');
        if (elTiempo) elTiempo.textContent = tiempoStr;

        // Ritmo (min/km)
        if (a.distance > 0) {
          const ritmoSeg = totalSeg / (a.distance / 1000);
          const ritmoMin = Math.floor(ritmoSeg / 60);
          const ritmoSegR = Math.round(ritmoSeg % 60).toString().padStart(2, '0');
          const elRitmo = document.getElementById('actRitmo');
          if (elRitmo) elRitmo.textContent = `${ritmoMin}:${ritmoSegR}`;
        }
        ...
        // Calorías y FC para métricas del dashboard
        const kcalQuem = a.calories || Math.round(totalSeg / 60 * 8);
        const elKcal = document.getElementById('mKcalQuem');
        const elFCm = document.getElementById('mFC');
        if (elKcal) elKcal.textContent = kcalQuem.toLocaleString();
