  ...
  // ══════════════════════════════════════════════
  const TERRA_WORKER = 'https://terra-worker.jaimea-gomezh.workers.dev';

  // ── Helpers de UI ──────────────────────────────
  function terraSet(id, valor) {
    const el = document.getElementById(id);
    if (el) el.textContent = valor;
  }

  function terraBar(barId, porcentaje) {
    const el = document.getElementById(barId);
    if (el) el.style.width = Math.min(100, Math.round(porcentaje)) + '%';
  }

  // Formatea minutos → "7h 23m"
  ...
  // ── Verifica si hay wearable conectado (localStorage) ─
  async function iniciarTerra() {
    // Detectar si venimos de redirect de Terra
    const params = new URLSearchParams(window.location.search);
    if (params.get('terra') === 'ok') {
      // Limpiar URL sin recargar
      window.history.replaceState({}, '', window.location.pathname);
      // Esperar un momento y buscar el user_id en el worker
      await new Promise(r => setTimeout(r, 1500));
      await terraRefreshUserId();
    }

    const terraUserId = localStorage.getItem('terra_user_id');
    if (terraUserId) {
      mostrarTerraConectado();
      await cargarDatosTerra();
    } else {
      // Intentar recuperar desde el worker por si ya estaba conectado
      await terraRefreshUserId();
    }
  }

  // Busca en el Worker si ya existe el terra_user_id para este Firebase user
  async function terraRefreshUserId() {
    try {
      const user = window._auth && window._auth.currentUser;
      if (!user) return;
      const ref = user.uid;
      const res = await fetch(`${TERRA_WORKER}/userid?ref=${encodeURIComponent(ref)}`);
      const data = await res.json();
      if (data.ok && data.user_id) {
        localStorage.setItem('terra_user_id', data.user_id);
        localStorage.setItem('terra_provider', data.provider || 'Wearable');
        mostrarTerraConectado();
        await cargarDatosTerra();
      }
    } catch (e) {
      console.warn('Terra: no se pudo recuperar user_id', e);
    }
  }

  // ── Abre el widget de Terra ────────────────────
  async function conectarWearable() {
    try {
      const user = window._auth && window._auth.currentUser;
      if (!user) { alert('Debes iniciar sesión primero.'); return; }

      const btn = document.querySelector('#terraNoConectado button');
      if (btn) { btn.textContent = 'Abriendo...'; btn.disabled = true; }

      const res = await fetch(`${TERRA_WORKER}/connect`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ reference_id: user.uid, email: user.email }),
      });
      const data = await res.json();
      ...
      if (data.ok && data.url) {
        // Abrir widget en popup
        const popup = window.open(data.url, 'TerraConnect',
          'width=480,height=640,menubar=no,toolbar=no,location=no,status=no');

        // Polling: cuando el popup cierre, buscar el user_id
        const poll = setInterval(async () => {
          if (!popup || popup.closed) {
            clearInterval(poll);
            await new Promise(r => setTimeout(r, 2000)); // dar tiempo al webhook
            await terraRefreshUserId();
          }
        }, 800);
      } else {
        alert('No se pudo iniciar la conexión con Terra. Verifica la configuración del Worker.');
        console.error('Terra error:', data);
      }
    } catch (e) {
      console.error('Terra conectar error:', e);
      alert('Error de conexión. Intenta de nuevo.');
      const btn = document.querySelector('#terraNoConectado button');
      if (btn) { btn.textContent = 'Conectar'; btn.disabled = false; }
    }
  ...
  async function desconectarWearable() {
    if (!confirm('¿Desconectar tu wearable?')) return;
    try {
      const user = window._auth && window._auth.currentUser;
      if (user) {
        await fetch(`${TERRA_WORKER}/disconnect`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ reference_id: user.uid }),
        });
      }
    } catch (e) { /* no es crítico */ }
    localStorage.removeItem('terra_user_id');
    localStorage.removeItem('terra_provider');
    document.getElementById('terraConectado').style.display  = 'none';
    document.getElementById('terraNoConectado').style.display = 'flex';
  }

  // ── Mostrar sección conectado ──────────────────
  function mostrarTerraConectado() {
    const provider = localStorage.getItem('terra_provider') || 'Wearable';
    document.getElementById('terraNoConectado').style.display = 'none';
    document.getElementById('terraConectado').style.display   = 'block';
    terraSet('terraProviderNombre', provider);
  }

  // ── Cargar y mostrar datos de Terra ───────────
  async function cargarDatosTerra() {
    try {
      const user = window._auth && window._auth.currentUser;
      if (!user) return;
      const ref = user.uid;

      // Cargar en paralelo: datos diarios + sueño
      const [dailyRes, sleepRes] = await Promise.all([
        fetch(`${TERRA_WORKER}/data?ref=${encodeURIComponent(ref)}&type=daily`),
        fetch(`${TERRA_WORKER}/data?ref=${encodeURIComponent(ref)}&type=sleep`),
      ]);

      const dailyJson = await dailyRes.json();
      const sleepJson = await sleepRes.json();

      // ── Datos diarios (pasos, calorías, FC reposo) ──
      if (dailyJson.ok && dailyJson.data && dailyJson.data.length > 0) {
        // Último registro disponible
        const d = dailyJson.data[dailyJson.data.length - 1];

        const steps    = d.distance_data?.steps ?? d.steps ?? null;
        const calsAct  = d.calories_data?.net_activity_calories
                      ?? d.calories_data?.active_calories
                      ?? d.active_calories ?? null;
        const fcReposo = d.heart_rate_data?.summary?.resting_hr_bpm
                      ?? d.resting_heart_rate_bpm ?? null;
        const hrv      = d.heart_rate_data?.summary?.avg_hrv_sdnn
                      ?? d.hrv_data?.last_night_5_min_high ?? null;

        if (steps !== null) {
          terraSet('terraSteps', steps.toLocaleString('es-CL'));
          terraBar('terraStepsBar', (steps / 10000) * 100);
          terraSet('terraStepsMeta', `meta: 10.000 · ${Math.round((steps/10000)*100)}%`);
        }
        if (calsAct !== null) {
          terraSet('terraCals', Math.round(calsAct).toLocaleString('es-CL'));
          terraBar('terraCalsBar', (calsAct / 800) * 100);
        }
        if (fcReposo !== null) {
          terraSet('terraFCReposo', Math.round(fcReposo));
          const trend = fcReposo < 60 ? '↓ Excelente' : fcReposo < 70 ? 'Normal' : '↑ Elevada';
          terraSet('terraFCReposoTrend', trend);
        }
        if (hrv !== null) {
          const hrvVal = Math.round(hrv);
          terraSet('terraHRV', hrvVal);
          const status = hrvVal > 60 ? '✓ Buena recuperación' : hrvVal > 40 ? 'Recuperación moderada' : '⚠ Recuperación baja';
          terraSet('terraHRVStatus', status);
        }
      }

      // ── Datos de sueño ──────────────────────────
      if (sleepJson.ok && sleepJson.data && sleepJson.data.length > 0) {
        const s = sleepJson.data[sleepJson.data.length - 1];

        const totalMin  = s.sleep_durations_data?.total_sleep_duration_minutes
                       ?? (s.sleep_durations_data?.duration?.asleep?.duration_light_sleep_state_minutes
                         + (s.sleep_durations_data?.duration?.asleep?.duration_deep_sleep_state_minutes ?? 0)
                         + (s.sleep_durations_data?.duration?.asleep?.duration_REM_sleep_state_minutes ?? 0))
                       ?? null;
        const deepMin   = s.sleep_durations_data?.duration?.asleep?.duration_deep_sleep_state_minutes ?? null;
        const remMin    = s.sleep_durations_data?.duration?.asleep?.duration_REM_sleep_state_minutes  ?? null;
        const score     = s.sleep_efficiency_percentage ?? s.sleep_score ?? null;

        if (totalMin !== null) terraSet('terraTotal', fmtMin(totalMin));
        if (deepMin  !== null) terraSet('terraDeep',  fmtMin(deepMin));
        if (remMin   !== null) terraSet('terraREM',   fmtMin(remMin));
        if (score    !== null) terraSet('terraSleepScore', Math.round(score));
      }

    } catch (e) {
      console.warn('Terra: error cargando datos', e);
    }
  }

  window.conectarWearable    = conectarWearable;
  window.desconectarWearable = desconectarWearable;
