function toggleMenu() {
  ...
  function cerrarLoginClick(e) {
    if (e.target === document.getElementById('modalLogin')) cerrarLogin();
  }

  function loginGoogle() {
    // Simulación — se conectará a Firebase
    abrirDashboard('Atleta');
  }
  ...
  function loginEmail() {
    ...
    const rawNombre = email.split('@')[0].replace(/[^a-zA-ZáéíóúÁÉÍÓÚñÑ]/g, ' ').trim();
    const nombre = rawNombre.charAt(0).toUpperCase() + rawNombre.slice(1);
    abrirDashboard(nombre || 'Atleta');
  }
  ...
  const STRAVA_REDIRECT   = encodeURIComponent('https://magnificent-vacherin-102e4c.netlify.app');
  const STRAVA_SCOPE      = 'read,activity:read_all';

  function conectarStrava() {
    const authUrl = `https://www.strava.com/oauth/authorize?client_id=${STRAVA_CLIENT_ID}&redirect_uri=https://magnificent-vacherin-102e4c.netlify.app&response_type=code&scope=${STRAVA_SCOPE}&approval_prompt=force`;
  ...
  async function cargarDatosStrava(accessToken) {
    try {
      ...
      if (actividades.length > 0) {
        ...
        const zona = fcProm > 160 ? 'Z4' : fcProm > 140 ? 'Z3' : 'Z2';
  ...
  function handleStravaCallback() {
    ...
    if (code) {
      // Mostrar aviso — el intercambio de código requiere backend
      // Por ahora guardamos que el usuario autorizó
      localStorage.setItem('strava_authorized', 'true');
      // Limpiar URL
      window.history.replaceState({}, document.title, window.location.pathname);
      alert('✅ Strava autorizado. La conexión completa estará activa cuando integremos el servidor.');
    }
  }
  ...
  function selectExerciseReal(exName, btn) {
    ...
    if (chartStrength) {
      ...
      // Hide points when too many sessions to avoid clutter
  ...
  function displayExerciseData(sessions, exName) {
    ...
    if (chartStrength) {
      chartStrength.data.labels = labels;
      chartStrength.data.datasets[0].data = weights;
      chartStrength.data.datasets[0].pointBackgroundColor = weights.map(w => w===best?'#d4a843':'rgba(212,168,67,0.5)');
      chartStrength.data.datasets[0].pointRadius = 0;
      chartStrength.update('active');
    }
  ...
  function selectExercise(ex, btn) {
    ...
    if (chartStrength) {
      ...
      chartStrength.data.datasets[0].pointBackgroundColor = d.data.map(v => v===best?'#d4a843':'rgba(212,168,67,0.5)');
      chartStrength.data.datasets[0].pointRadius = 0;
      chartStrength.update('active');
    }
    // Actualizar tabla
...
function abrirLogin() { document.getElementById('loginOverlay').classList.add('active'); document.body.style.overflow='hidden'; }
  function cerrarLogin() { document.getElementById('loginOverlay').classList.remove('active'); document.body.style.overflow=''; limpiarMensaje(); }
  function cerrarLoginOverlay(e) { if(e.target===document.getElementById('loginOverlay')) cerrarLogin(); }
  function cambiarTab(tab,btn) {
    document.querySelectorAll('.login-tab').forEach(t=>t.classList.remove('active'));
    document.querySelectorAll('.login-form').forEach(f=>f.classList.remove('active'));
    btn.classList.add('active');
    document.getElementById(tab==='login'?'formLogin':'formRegistro').classList.add('active');
    limpiarMensaje();
  }
  function mostrarMensaje(texto,tipo) { const m=document.getElementById('loginMsg'); m.textContent=texto; m.className='login-msg '+tipo; }
  function limpiarMensaje() { const m=document.getElementById('loginMsg'); m.className='login-msg'; m.textContent=''; }
  function traducirError(code) {
    const e={ 'auth/user-not-found':'No existe una cuenta con ese correo.','auth/wrong-password':'Contraseña incorrecta.','auth/invalid-email':'El correo no es válido.','auth/email-already-in-use':'Ya existe una cuenta con ese correo.','auth/weak-password':'La contraseña debe tener al menos 6 caracteres.','auth/too-many-requests':'Demasiados intentos. Intenta más tarde.','auth/popup-closed-by-user':'Cerraste la ventana de Google sin completar.','auth/network-request-failed':'Error de red. Revisa tu conexión.' };
    return e[code]||'Ocurrió un error. Intenta nuevamente.';
  }
  async function iniciarSesionEmail() {
    ...
    try { await window._signInWithEmailAndPassword(window._auth,email,password); mostrarMensaje('¡Bienvenido de vuelta!','success'); setTimeout(cerrarLogin,1000); }
    catch(e) { mostrarMensaje(traducirError(e.code),'error'); }
  }
  async function registrarse() {
    const email=document.getElementById('regEmail').value.trim();
    const pass=document.getElementById('regPassword').value;
    const confirm=document.getElementById('regPasswordConfirm').value;
    if(!email||!pass||!confirm) return mostrarMensaje('Completa todos los campos.','error');
    if(pass!==confirm) return mostrarMensaje('Las contraseñas no coinciden.','error');
    if(pass.length<6) return mostrarMensaje('La contraseña debe tener al menos 6 caracteres.','error');
    try { await window._createUserWithEmailAndPassword(window._auth,email,pass); mostrarMensaje('¡Cuenta creada! Bienvenido.','success'); setTimeout(cerrarLogin,1200); }
    catch(e) { mostrarMensaje(traducirError(e.code),'error'); }
  }
  async function iniciarSesionGoogle() {
    try { await window._signInWithPopup(window._auth,window._googleProvider); mostrarMensaje('¡Sesión iniciada!','success'); setTimeout(cerrarLogin,800); }
    catch(e) { mostrarMensaje(traducirError(e.code),'error'); }
  }
  async function cerrarSesion() { await window._signOut(window._auth); }
